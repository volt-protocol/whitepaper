...
* [Surplus Buffer Ratio](#surplus-buffer-ratio)
...
## Surplus Buffer Ratio
Based on the time-weighted average surplus buffer vs time-weighted average VOLT supply, the VOLT rate may be lower than the baserate.

When the surplus buffer ratio is too low, more yield will be retained by the system to drive surplus buffer growth and encourage holders to redeem their VOLT to reduce leverage. The minimum yield paid to VOLT holders when the surplus buffer is at the minimum allowed size is zero, there are never negative rates. The change in rate is linear in proportion to target time-weighted average surplus buffer vs actual time-weighted average surplus buffer.

## Reserves Ratio
Maintaining sufficient liquid reserves to support redemptions at peg is the key goal of the VOLT rate. Liquid reserves refers to both stablecoins lent in venues like Compound or Aave (redeemable on demand) and cash balances in the PSM.

When the liquid portion of the PCV is too small, the VOLT rate may be higher than the baserate. A higher rate will both incentivize new deposits to maintain liquidity, and punish VCON holders who have allocated into illiquid instruments at yields below the protocol's cost of capital.

This model is more likely to have a kink, as it becomes increasingly urgent to attract new reserves deposits below a minimum threshold.

## VCON Revenue Split

Yield generated by the system in excess of the VOLT rate accrues to the surplus buffer. When a VCON holder generates yield for the surplus buffer, or takes losses vs the VOLT rate, their VCON balance is adjusted accordingly.
