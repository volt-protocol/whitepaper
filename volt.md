...
The VCON governance token will be launched in a liquidity bootstrapping pool (LBP) on Balancer via Copper Launch. An LBP is a mechanism for on chain price discovery of a new token. The LBP will be seeded with VCON governance tokens and protocol controlled WETH. The LBP determines what the starting price of the VCON token is at launch. This VCON/WETH liquidity will remain permanently as PCV. WWETH was chosen as the pair given that Copper supports WETH, DAI, or USDC, and of these we expect VCON to share the most correlation with WETH. The LBP will start at a market cap of $250m for VCON or $0.25 per VCON token and last for a week. The pool will be seeded with 10% of the VCON supply, 100m VCON tokens and ~96 WETH at a 99/1 weighting.
